...
# --------------------------------------------
# 2. Bitcoin Verisini Hugging Face'ten Çekme
# --------------------------------------------
btc_api = "https://datasets-server.huggingface.co/rows?dataset=gauss314%2Fbitcoin_daily&config=default&split=train&offset=0&length=300"
response_btc = requests.get(btc_api)
if response_btc.status_code != 200:
    raise Exception("Bitcoin verisi API çağrısı başarısız oldu!")
btc_json = response_btc.json()

# satırları alıp DataFrame'e dönüştürelim
btc_rows = btc_json.get("rows", [])
btc_data = [r["row"] for r in btc_rows if "row" in r]
df_btc = pd.DataFrame(btc_data)

print("\nBitcoin veri seti sütunları:", df_btc.columns)

# Beklenen sütunlar: date, close
if not {"date", "close"}.issubset(df_btc.columns):
    raise Exception("Bitcoin verisinde 'date' veya 'close' sütunu bulunamadı!")
